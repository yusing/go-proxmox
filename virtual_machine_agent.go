...
func (v *VirtualMachine) AgentCommand(ctx context.Context, command string) (map[string]any, error) {
	body := map[string]any{
		"command": command,
	}
	var resp struct {
...
func (v *VirtualMachine) AgentFstrim(ctx context.Context) (map[string]any, error) {
	var resp struct {
		Result map[string]any `json:"result"`
	}
