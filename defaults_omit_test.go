...
func TestContainerConfig_ExplicitValuesAppearOnTheWire(t *testing.T) {
	cfg := ContainerConfig{
		Arch:     new("amd64"),
		CMode:    new("tty"),
		Console:  new(IntOrBool(false)), // explicit disable — must reach PVE as 0, not be silently swallowed
		CPUUnits: new(1024),
		Memory:   new(512),
		Swap:     new(0), // explicit no-swap — must reach PVE as 0, not be omitted
		TTY:      new(2),
	}
...
func TestVirtualMachineConfig_ExplicitFalseSurvivesForBooleanFields(t *testing.T) {
	...
	cfg := VirtualMachineConfig{
		KVM:       new(IntOrBool(false)),
		Acpi:      new(IntOrBool(false)),
		CIUpgrade: new(IntOrBool(false)),
	}
	m := marshalToMap(t, cfg)
...
func TestVirtualMachineBackupOptions_ExplicitDisablesStillReachTheWire(t *testing.T) {
	...
	opts := VirtualMachineBackupOptions{
		VMID:        100,
		Remove:      new(IntOrBool(false)),
		StdExcludes: new(IntOrBool(false)),
	}
...
func TestFirewallNodeOption_ExplicitEnableMarshalsAsOne(t *testing.T) {
	...
	opts := FirewallNodeOption{Enable: new(IntOrBool(true))}
	m := marshalToMap(t, opts)
...
func TestFirewallClusterOption_ExplicitEbtablesDisableSurvives(t *testing.T) {
	...
	opts := FirewallClusterOption{Ebtables: new(IntOrBool(false))}
	m := marshalToMap(t, opts)
...
func TestFirewallClusterOption_ExplicitValuesMarshal(t *testing.T) {
	...
	opts := FirewallClusterOption{Enable: 1, Ebtables: new(IntOrBool(true))}
	m := marshalToMap(t, opts)
