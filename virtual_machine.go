...
func (v *VirtualMachine) HasTag(value string) bool {
	...
	return slices.Contains(v.VirtualMachineConfig.TagsSlice, value)
}
...
func (v *VirtualMachine) DeleteFirewallIPSet(ctx context.Context, name string, force bool) error {
	return v.client.Delete(ctx, fmt.Sprintf("/nodes/%s/qemu/%d/firewall/ipset/%s", v.Node, v.VMID, name), map[string]any{"force": force})
}
...
func (v *VirtualMachine) DeleteFirewallIPSetEntry(ctx context.Context, name string, cidr string, digest string) error {
	return v.client.Delete(ctx, fmt.Sprintf("/nodes/%s/qemu/%d/firewall/ipset/%s/%s", v.Node, v.VMID, name, cidr), map[string]any{
		"digest": digest,
	})
}
...
func (s *VirtualMachineSnapshot) Config(ctx context.Context) (config map[string]any, err error) {
